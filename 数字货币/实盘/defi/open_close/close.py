...
# 初始参数
tickers = ['QNTUSDT', 'ETHUSDT', 'ENSUSDT', 'LINKUSDT', 'MATICUSDT', 'BNBUSDT']
basis_out = [-0.06, -1, -0.01, -0.005, -0.0006, -0.4]
spot_init_price = [121.1, 1181.57, 11.07, 6.742, 0.9371, 281.5]
p_decimal = [1, 2, 2, 3, 4, 1]
vol_each = np.array([10, 2, 200, 100, 500, 5])
vol_end = np.array([10, 2, 200, 100, 500, 7])*-1
...
def on_quote(context, quote):
    ...
    # 期货行情
    if quote.exchange_id == future["exchange"]:
    ...
    # 期货小于现货 + 期货仓位大于目标仓位——挂单开仓/maker
    if context.basis[k] < basis_out[k] and \
        ...
        future_id = context.insert_order(future["ticker"][k], future["exchange"], future["account"],
                                         future["bid"][k], int(vol_each[k] * 1000), PriceType.Limit,
                                         Side.Buy, Offset.Close)
        ...
        # context.log.info(f"[CURRENT BASIS:] {context.basis}")
        context.log.info(f"MAKE ORDER BUY CLOSE:[{future_id}], [VOLUME:] {int(vol_each[k])} [FUTURE]")

    # 订单超时
    if context.now() - context.start[k] > (6 * (10 ** 9)) and context.future_id[k] is not None:
        # 撤单
...
def on_order(context, order):
    ...
    if order.status == OrderStatus.Filled and order.exchange_id == future["exchange"]:
        ...
        order_id = context.insert_order(spot["ticker"][k], spot["exchange"], spot["account"],
                                        round(spot["last"][k] * 0.98, p_decimal[k]), int(vol_each[k] * 10000),
                                        PriceType.Limit, Side.Sell, Offset.Close)
        # 报告买入订单
        context.log.info(f"MAKE ORDER SELL CLOSE:[{order_id}], [VOLUME:] {vol_each[k]} [SPOT]")
    ...
    if order.status == OrderStatus.Cancelled and \
        ...
        if order.volume_left < vol_each[k] * 1000 and \
            ...
            if make_up * spot_init_price[k] > 100:
                # 卖平现货(last-1价位)——taker
                order_id = context.insert_order(spot["ticker"][k], spot["exchange"], spot["account"],
                                                round(spot["last"][k] * 0.98, p_decimal[k]), int(make_up * 10000),
                                                PriceType.Limit, Side.Sell, Offset.Close)
                # 报告卖出订单
                context.log.info(f"[{tickers[k]}][MAKE ORDER SELL CLOSE]:[{order_id}], [VOLUME:] {make_up} [SPOT]")
...
def on_seldefposition(context, seldefposition):
    ...
    if seldefposition.exchange_id == future["exchange"]:
        ...
        context.log.info(f"[{tickers[k]} remains:] "
                         f"{round((context.real_position[k] - context.target_position[k]) / context.target_position[k], 2)}")
